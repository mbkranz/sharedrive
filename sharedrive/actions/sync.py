...
def sync_resource_in_descriptor(
    descriptor: Path | str,
    resource_name: str,
    *,
    dry_run: bool = False,
    ...
    """Sync one top-level resource into nested descriptor resources."""
    ...
    if adapter_name != "googledrive":
        raise NotImplementedError(
            "sync is currently implemented only for Google Drive resources."
        )
    ...
    if entity_type == "Directory":
        ...
        raise NotImplementedError(
            f"sync is not implemented for entityType '{entity_type or 'unknown'}'."
        )
    ...
    if log is not None:
        ...
        log(
            f"{verb} {len(child_resources)} resource(s) for resource '{resolved_name}'."
        )
...
def sync_package_resource_in_descriptor(
    ...
    *,
    ...
    """Backward-compatible alias for sync_resource_in_descriptor."""
    return sync_resource_in_descriptor(
